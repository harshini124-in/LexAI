...
The Loop
User uploads a document OR requests a new one to be generated.
LoopholeHound attacks the document and generates a vulnerability report with an "Exploitability Score".
DocumentCraft reads the report and rewrites the document to patch the vulnerabilities.
This cycle repeats (up to 3 rounds) until the Exploitability Score drops below a safe threshold.
The final document is presented to the user alongside a detailed compliance radar and battle log.
✨ Features
